...
const NetworkGraph = () => {
    ...
    const renderGraph = (data) => {
        ...
        data.forEach(device => {
            ...
            if (isLocal && device.ip !== GATEWAY_IP) {
                links.push({ source: device.ip, target: GATEWAY_IP });
            }

            device.connections.forEach(conn => {
                const isDstLocal = conn.dst_ip.startsWith("192.168.");
                const isSrcLocal = conn.src_ip.startsWith("192.168.");
                ...
                if (isSrcLocal && !isDstLocal) {
                    links.push({ source: conn.src_ip, target: GATEWAY_IP });
                    links.push({ source: GATEWAY_IP, target: conn.dst_ip });
        ...
        const simulation = d3.forceSimulation(nodes)
            ...
            .force("x", d3.forceX(d => d.id === GATEWAY_IP ? width / 2 : (d.group === "remote" ? width * 0.75 : width * 0.25)).strength(0.5))
        ...
        nodeGroup.append("circle")
            .attr("r", 10)
            .attr("fill", d => d.group === "gateway" ? "orange" : d.group === "lan" ? "#ff6666" : "purple");
        ...
        linkPathsGroupRef.current = svg.append("g");
    ...
    useEffect(() => {
        ...
        if (src_ip !== gateway && dst_ip !== gateway) {
            if (nodeMap.has(gateway)) points.push(nodeMap.get(gateway));
        }
